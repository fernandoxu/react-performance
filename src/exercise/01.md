...
Code splitting acts on the principle(原则) that loading less code will speed up
your app. Say for example that we're building a complex dashboard application
that includes the venerable(受尊敬) d3 library for graphing data. Your users
start complaining(抱怨) because it takes too long to load the login screen.
...
when the user loads the login screen? Nope! We could load that on-demand 按需.
...
For this one, you'll need to open the final in isolation(隔离) and open the
Chrome DevTools Network tab to watch the webpack chunks load when you click
"show globe." Your objective is to have the network load those same chunks so
they're not in the bundle to begin with.
...
🦉 You may also want to use Incognito(伪装/匿名) mode so your browser plugins
don't mess with the typical user experience.
...
interact(互动) with our globe. We don't want to have to make them wait first to
...
have webpack instruct(指示) the browser to prefetch dynamic imports:
