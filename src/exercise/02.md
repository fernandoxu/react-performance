...
Elaborate(详细说明) on your learnings here in `src/exercise/02.md`

## Background

React hooks are amazing. Being able to put all the logic and state management
within a function component (the "render method" equivalent(等同) for a class)
allows for mind blowing composability.

This power comes with an unfortunate limitation that calculations performed
within render will be performed every single render, regardless(无论如何) of
whether the inputs for the calculations change. For example:
...
when the result actually needs to be re-evaluated(估计) (when the dependencies
