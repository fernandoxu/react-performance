...
the most surgically optimal(外科手术般最佳的) updates to the DOM to speed things
up for us big-time.
...
tips about certain(某些) parts of the React tree when there's a state update.
You can opt-out of state updates for a part of the React tree by using one of
